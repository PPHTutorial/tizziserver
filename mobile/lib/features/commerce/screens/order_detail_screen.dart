...
class OrderDetailScreen extends ConsumerWidget {
    ...
    return Scaffold(
      ...
      body: async.when(
        ...
        error: (e, _) => CenteredState.error(
          ...
          action: PrimaryButton(label: 'Retry', onPressed: () => ref.invalidate(orderDetailProvider(orderId))),
        ),
        data: (o) => MaxWidth(
          child: ListView(
          ...
          children: [
            Row(
              children: [
                Text(o.number, style: context.text.titleLarge),
                const SizedBox(width: AppSpace.s8),
                _pill(context, o.status),
              ],
            ),
            const SizedBox(height: AppSpace.s4),
            Text('${o.fulfilmentMethod == 'PICKUP' ? 'Pickup' : 'Delivery'} · paid by ${o.paymentMethod ?? '—'}',
                style: context.text.bodyMedium?.copyWith(color: c.textMed)),
            const SizedBox(height: AppSpace.s20),

            for (final vo in o.vendorOrders) _VendorBlock(vo: vo, currency: o.currency, orderId: o.id),

            const SizedBox(height: AppSpace.s8),
            Text('Payment', style: context.text.titleMedium),
            const SizedBox(height: AppSpace.s8),
            _row(context, 'Items', formatMoney(o.itemsSubtotalMinor, o.currency)),
            if (o.discountMinor > 0) _row(context, 'Discount', '-${formatMoney(o.discountMinor, o.currency)}'),
            _row(context, o.fulfilmentMethod == 'PICKUP' ? 'Pickup' : 'Delivery', formatMoney(o.deliveryFeeMinor, o.currency)),
            _row(context, 'Service fee', formatMoney(o.serviceFeeMinor, o.currency)),
            if (o.taxMinor > 0) _row(context, 'Tax', formatMoney(o.taxMinor, o.currency)),
            const Divider(height: AppSpace.s24),
            _row(context, 'Total', formatMoney(o.totalMinor, o.currency), bold: true),
            if (o.invoiceNumber != null) ...[
              const SizedBox(height: AppSpace.s12),
              _InvoiceButton(orderId: o.id, invoiceNumber: o.invoiceNumber!),
            ],
              ...
              Padding(
                ...
                child: Row(
                  children: [
                    ...
                    const Spacer(),
                    Text(_short(e.at), style: context.text.labelSmall?.copyWith(color: c.textLow)),
                  ],
                ),
              ),
  ...
  static String _short(String iso) => iso.length >= 16 ? iso.substring(0, 16).replaceFirst('T', ' ') : iso;
  ...
  Widget _pill(BuildContext context, String status) => StatusBadge(status, tone: _orderStatusTone(status));
}
...
class _VendorBlock extends ConsumerWidget {
  const _VendorBlock({required this.vo, required this.currency, required this.orderId});
        ...
        children: [
          Row(
            children: [
              ...
              Expanded(child: Text(vo.vendorName, style: context.text.titleSmall)),
              Text(vo.status.replaceAll('_', ' ').toLowerCase(),
                  style: context.text.labelSmall?.copyWith(color: c.textMed)),
            ],
          ),
          if (vo.pickupCode != null) ...[
            const SizedBox(height: 4),
            Text('Pickup code: ${vo.pickupCode}', style: context.text.labelSmall?.copyWith(color: c.primary)),
          ],
          const SizedBox(height: AppSpace.s8),
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  Text(formatMoney(it.totalMinor, currency), style: context.text.bodyMedium),
                ],
              ),
            ),
          Row(
            children: [
              if (vo.deliveryId != null)
                OutlinedButton.icon(
                  icon: const Icon(AppIcons.local_shipping_outlined, size: 18),
                  label: const Text('Track'),
                  onPressed: () => context.push(RoutePaths.delivery(vo.deliveryId!)),
                ),
              const SizedBox(width: AppSpace.s8),
              OutlinedButton.icon(
                ...
                onPressed: () async {
                  try {
                    final cid = await ref.read(stallApiProvider).conversationForOrder(orderId);
                    if (context.mounted) context.push(RoutePaths.conversation(cid), extra: vo.vendorName);
                  } catch (e) {
              ...
              if (vo.status == 'COMPLETED') ...[
                ...
                OutlinedButton.icon(
                  ...
                  onPressed: () => showModalBottomSheet<void>(
                    ...
                    builder: (_) => _ReturnRequestSheet(orderId: orderId, vo: vo),
                  ),
                ),
              ],
            ],
...
class _ReturnRequestSheetState extends ConsumerState<_ReturnRequestSheet> {
  late final Map<String, int> _qty = {for (final it in widget.vo.items) if (it.id != null) it.id!: 0};
  ...
  int _returned(String orderItemId) => widget.vo.returns
      ...
      .fold(0, (n, r) => n + r.items.where((i) => i.orderItemId == orderItemId).fold(0, (m, i) => m + i.qty));
  ...
  Future<void> _submit() async {
    final items = _qty.entries.where((e) => e.value > 0).map((e) => (orderItemId: e.key, qty: e.value)).toList();
    ...
    try {
      await ref.read(stallApiProvider).requestReturn(
            widget.orderId,
            widget.vo.id,
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Return requested — the seller will review it')),
        );
      }
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
                      ...
                      children: [
                        Expanded(
                          child: Text(
                            ...
                            maxLines: 2,
                            overflow: TextOverflow.ellipsis,
                            style: remaining <= 0 ? context.text.bodyMedium?.copyWith(color: c.textLow) : context.text.bodyMedium,
                          ),
                        ),
                        if (remaining <= 0)
                          Text('fully returned', style: context.text.labelSmall?.copyWith(color: c.textLow))
                        else ...[
                          IconButton(
                            icon: const Icon(AppIcons.remove, size: 20),
                            onPressed: qty > 0 ? () => setState(() => _qty[it.id!] = qty - 1) : null,
                          ),
                          Text('$qty', style: context.text.titleSmall),
                          IconButton(
                            icon: const Icon(AppIcons.add, size: 20),
                            onPressed: qty < remaining ? () => setState(() => _qty[it.id!] = qty + 1) : null,
                          ),
                        ],
                      ],
                    ),
            ...
            PrimaryButton(label: _busy ? 'Submitting…' : 'Submit return', loading: _busy, onPressed: _busy ? null : _submit),
          ],
        ),
      ),
    );
...
class _CancelButtonState extends ConsumerState<_CancelButton> {
  ...
  Future<void> _cancel() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Order cancelled — refunded to your wallet')),
        );
      }
    } on StallApiException catch (e) {
      setState(() => _error = e.message);
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        SecondaryButton(label: _busy ? 'Cancelling…' : 'Cancel order', onPressed: _busy ? null : _cancel),
...
class _InvoiceButtonState extends ConsumerState<_InvoiceButton> {
  ...
  Future<void> _download() async {
    ...
    try {
      final bytes = await ref.read(stallApiProvider).orderInvoiceBytes(widget.orderId);
