              ...
              child: async.when(
                ...
                error: (e, _) => CenteredState.error(
                  ...
                  action: PrimaryButton(
                    label: 'Retry',
                    onPressed: () =>
                        ref.invalidate(sellerOrderProvider(vendorOrderId)),
                  ),
                ),
...
class _DetailState extends ConsumerState<_Detail> {
      ...
      child: ListView(
        ...
        children: [
          Row(
            children: [
              Expanded(
                child: Text(o.orderNumber, style: context.text.titleLarge),
              ),
                  ...
                  Padding(
                    ...
                    child: Row(
                      children: [
                        ...
                        Text(
                          formatMoney(it.totalMinor, o.currency),
          ...
          _row(
            context,
            'Items subtotal',
            formatMoney(o.subtotalMinor, o.currency),
          ),
          _row(
            context,
            'Platform commission',
            '-${formatMoney(o.commissionMinor, o.currency)}',
          ),
          const Divider(height: AppSpace.s24),
          _row(
            context,
            'Payout to you',
            formatMoney(o.payoutMinor, o.currency),
            bold: true,
          ),
          Text(
            o.status == 'COMPLETED'
  ...
  Widget _doneBanner(BuildContext context, String label, Color tone) =>
      Container(
        padding: const EdgeInsets.all(AppSpace.s16),
  ...
  Future<void> _review(String decision) async {
    ...
    try {
      final result = await ref
          .read(stallApiProvider)
          .reviewReturn(widget.ret.id, decision: decision);
...
class _PickupCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final live =
        order.status == 'READY_FOR_PICKUP' || order.status == 'HANDED_OVER';
    return AppCard(
      padding: const EdgeInsets.all(AppSpace.s16),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(
            'Pickup code',
            style: context.text.labelMedium?.copyWith(color: c.textMed),
          ),
      ...
      child: async.when(
        ...
        error: (e, _) => Row(
          children: [
            ...
            TextButton(
              onPressed: () =>
                  ref.invalidate(sellerHandoffDeliveryProvider(deliveryId)),
        ...
        data: (d) {
          ...
          return Column(
            ...
            children: [
              ...
              if (courier == null)
                Text(
                  'Finding a courier — this usually takes a few minutes.',
                  style: context.text.bodyMedium?.copyWith(color: c.textMed),
                )
              else ...[
                Row(
                  children: [
                    CircleAvatar(
                      ...
                      child: Text(
                        courier.name.isEmpty
                            ? '?'
                            : courier.name.characters.first.toUpperCase(),
                          ...
                          Text(
                            [
                              ...
                              if (courier.vehicle != null)
                                [
                                      courier.vehicle!.type.toLowerCase(),
