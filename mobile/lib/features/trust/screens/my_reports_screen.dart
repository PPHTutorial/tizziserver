    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        ...
        child: async.when(
          ...
          error: (e, _) => AppErrorView(e, onRetry: () => ref.invalidate(myReportsProvider)),
          data: (list) => list.isEmpty
              ? const EmptyState(
                  icon: AppIcons.flag,
                  title: 'No reports filed',
                  message: 'If something on Stall seems wrong, report it and we\'ll look into it.',
                )
              : ListView.separated(
                  padding: const EdgeInsets.all(AppSpace.s16),
                  itemCount: list.length,
                  separatorBuilder: (_, __) => const Divider(height: AppSpace.s16),
                  itemBuilder: (context, i) {
                    final r = list[i];
                    return ListTile(
                      contentPadding: EdgeInsets.zero,
                      leading: Icon(reportTargetIcon(r.targetType), color: context.colors.textMed),
                      title: Text(r.category, style: context.text.bodyLarge),
                      subtitle: Text(
                      ...
                      trailing: StatusBadge(r.status, tone: _tone(r.status)),
                    );
                  },
                ),
        ),
