...
class StallApi {
  ...
  Future<Map<String, dynamic>> _send(
    ...
    try {
      res = await _dio.request<dynamic>(
        ...
        options: Options(method: method, headers: {..._headers(auth: auth), ...?extraHeaders}),
      );
    ...
    if (res.statusCode == 401 && auth && canRetry && _tokens.hasSession) {
      ...
      if (ok) {
        return _send(method, path, body: body, query: query, auth: auth, canRetry: false, extraHeaders: extraHeaders);
      }
  ...
  Future<AddressDto> saveAddress(Map<String, dynamic> body, {String? id}) async {
    final d = id == null
  ...
  Future<void> deleteAddress(String id) => _send('DELETE', '/api/v1/me/addresses/$id');
  ...
  Future<PaymentMethodDto> addPaymentMethod({
    ...
    bool makeDefault = false,
      ...
      PaymentMethodDto.fromJson(await _send('POST', '/api/v1/me/payment-methods', body: {
        'gateway': gateway,
        'token': token,
        if (brand != null) 'brand': brand,
  ...
  Future<void> removePaymentMethod(String id) => _send('DELETE', '/api/v1/me/payment-methods/$id');

  // --- Phase 3: cart -----------------------------------------------
  Future<CartDto> cart() async => CartDto.fromJson(await _send('GET', '/api/v1/cart'));

  Future<CartDto> addToCart(String offerId, {String? variantId, int qty = 1}) async => CartDto.fromJson(
        await _send('POST', '/api/v1/cart/items', body: {
          'offerId': offerId,
  ...
  Future<CartDto> updateCartItem(String itemId, {int? qty, bool? savedForLater}) async => CartDto.fromJson(
        await _send('PATCH', '/api/v1/cart/items/$itemId', body: {
          if (qty != null) 'qty': qty,
  ...
  Future<CartDto> clearCart() async => CartDto.fromJson(await _send('DELETE', '/api/v1/cart'));

  Future<CartDto> applyCoupon(String code) async =>
      CartDto.fromJson(await _send('POST', '/api/v1/cart/coupon', body: {'code': code}));

  Future<CartDto> removeCoupon() async => CartDto.fromJson(await _send('DELETE', '/api/v1/cart/coupon'));
  ...
  Future<CouponEvalDto> validateCoupon(String code) async =>
      CouponEvalDto.fromJson(await _send('POST', '/api/v1/coupons/validate', body: {'code': code}));

  // --- Phase 3: checkout + orders -----------------------------
  Future<CheckoutQuoteDto> checkoutQuote({String? fulfilmentMethod, String? couponCode}) async => CheckoutQuoteDto.fromJson(
        await _send('POST', '/api/v1/checkout/quote', body: {
    ...
    required String idempotencyKey,
      ...
      OrderDto.fromJson(await _send(
        ...
        body: {
          ...
          if (couponCode != null) 'couponCode': couponCode,
          'payment': {'method': paymentMethod, if (gateway != null) 'gateway': gateway},
        },
      ));

  Future<List<OrderCardDto>> orders({String? status, String? cursor}) async {
    final d = await _send('GET', '/api/v1/orders', query: {
  ...
  Future<OrderDto> order(String id) async => OrderDto.fromJson(await _send('GET', '/api/v1/orders/$id'));
  ...
  Future<({String id, String status, int amountMinor})> requestReturn(
    ...
    final d = await _send('POST', '/api/v1/orders/$orderId/return', body: {
      'vendorOrderId': vendorOrderId,
      'reason': reason,
      'items': items.map((it) => {'orderItemId': it.orderItemId, 'qty': it.qty}).toList(),
    });
    return (
      id: (d['id'] as String?) ?? '',
  ...
  Future<List<int>> orderInvoiceBytes(String orderId) async {
    ...
    try {
      ...
      throw StallApiException(code: 'NETWORK', message: e.message ?? 'Network error — check your connection.');
    }
    if ((res.statusCode ?? 500) >= 400) {
      ...
      throw StallApiException(code: code, message: message, status: res.statusCode);
    }
    return res.data ?? const [];
  }

  // --- Phase 3: wallet ------------------------------------------
  Future<WalletDto> wallet() async => WalletDto.fromJson(await _send('GET', '/api/v1/wallet'));

  Future<List<WalletTxnDto>> walletTransactions({String? cursor}) async {
    final d = await _send('GET', '/api/v1/wallet/transactions', query: {if (cursor != null) 'cursor': cursor});
    return (d['items'] as List<dynamic>? ?? const [])
  ...
  Future<WalletDto> walletTopUp(int amountMinor, {String? gateway}) async {
    final d = await _send(
      'POST',
      '/api/v1/wallet/topup',
      extraHeaders: {'Idempotency-Key': 'topup-${DateTime.now().microsecondsSinceEpoch}'},
      body: {'amountMinor': amountMinor, if (gateway != null) 'gateway': gateway},
    );
    return WalletDto(currency: 'GHS', balanceMinor: (d['balanceMinor'] as num?)?.toInt() ?? 0, pinRequired: true);
  }

  Future<int> walletWithdraw({required int amountMinor, required String pin}) async {
    final d = await _send('POST', '/api/v1/wallet/withdraw', body: {'amountMinor': amountMinor, 'pin': pin});
    return (d['balanceMinor'] as num?)?.toInt() ?? 0;
  }

  // --- Phase 4: customer delivery + tracking -----------------------

  Future<List<DeliveryDto>> deliveries({bool active = false, String? cursor}) async {
    final d = await _send('GET', '/api/v1/deliveries', query: {
      if (active) 'active': '1',
      if (cursor != null) 'cursor': cursor,
    });
  ...
  Future<DeliveryTrackDto> deliveryTrack(String id) async =>
      DeliveryTrackDto.fromJson(await _send('GET', '/api/v1/deliveries/$id/track'));
  ...
  Future<DeliveryEstimateDto> deliveryEstimate({
    ...
    String? vehicleType,
      ...
      DeliveryEstimateDto.fromJson(await _send('POST', '/api/v1/deliveries/estimate', body: {
        'pickup': {'lat': pickupLat, 'lng': pickupLng},
        'dropoff': {'lat': dropLat, 'lng': dropLng},
        if (vehicleType != null) 'vehicleType': vehicleType,
      }));

  Future<DeliveryDto> createDelivery(Map<String, dynamic> body) async => DeliveryDto.fromJson(
        await _send('POST', '/api/v1/deliveries',
            extraHeaders: {'Idempotency-Key': 'dlv-${DateTime.now().microsecondsSinceEpoch}'}, body: body),
      );

  Future<DeliveryDto> cancelDelivery(String id, {String? reason}) async => DeliveryDto.fromJson(
        await _send('POST', '/api/v1/deliveries/$id/cancel', body: {if (reason != null) 'reason': reason}),
      );

  Future<void> rateDelivery(String id, {required int stars, List<String>? tags, String? comment}) => _send(
        'POST',
        '/api/v1/deliveries/$id/rate',
        body: {'stars': stars, if (tags != null) 'tags': tags, if (comment != null) 'comment': comment},
      );

  Future<void> disputeDelivery(String id, {required String category, required String body}) =>
      _send('POST', '/api/v1/deliveries/$id/dispute', body: {'category': category, 'body': body});

  // --- Phase 4: courier ------------------------------------------

  Future<CourierMeDto> courierMe() async => CourierMeDto.fromJson(await _send('GET', '/api/v1/courier/me'));

  Future<Map<String, dynamic>> courierOnboard({String? firstName, String? lastName, bool agreementAccepted = false}) =>
      _send('POST', '/api/v1/courier/onboarding', body: {
        if (firstName != null) 'firstName': firstName,
        if (lastName != null) 'lastName': lastName,
        'agreementAccepted': agreementAccepted,
      });

  Future<void> courierSubmitKyc({required List<Map<String, String>> documents, String? selfieKey}) =>
      _send('POST', '/api/v1/courier/kyc', body: {'documents': documents, if (selfieKey != null) 'selfieKey': selfieKey});
  ...
  Future<Map<String, dynamic>> courierAddVehicle({
    ...
    int? year,
      ...
      _send('POST', '/api/v1/courier/vehicles', body: {
        'type': type,
  ...
  Future<void> courierUpdateVehicle(
    ...
    int? year,
      ...
      _send('PATCH', '/api/v1/courier/vehicles/$id', body: {
  ...
  Future<void> courierRemoveVehicle(String id) => _send('DELETE', '/api/v1/courier/vehicles/$id');
  ...
  Future<void> courierAddVehicleDocument(
    ...
    String? expiresAt,
      ...
      _send('POST', '/api/v1/courier/vehicles/$id/documents', body: {
        'type': type,
        'fileKey': fileKey,
  ...
  Future<void> courierUpsertServiceArea({
    ...
    bool? enabled,
      ...
      _send('POST', '/api/v1/courier/service-areas', body: {
        if (id != null) 'id': id,
        'name': name,
        'centerLat': centerLat,
        'centerLng': centerLng,
  ...
  Future<CourierDashboardDto> courierDashboard() async =>
      CourierDashboardDto.fromJson(await _send('GET', '/api/v1/courier/dashboard'));

  Future<Map<String, dynamic>> courierPerformance() => _send('GET', '/api/v1/courier/performance');

  Future<String> courierGoOnline(double lat, double lng) async {
    final d = await _send('POST', '/api/v1/courier/online', body: {'lat': lat, 'lng': lng});
  ...
  Future<void> courierHeartbeat(double lat, double lng, {double? heading, double? speed}) => _send(
        'POST',
        '/api/v1/courier/heartbeat',
        body: {'lat': lat, 'lng': lng, if (heading != null) 'heading': heading, if (speed != null) 'speed': speed},
      );

  Future<List<JobCardDto>> courierJobs({double? lat, double? lng}) async {
    final d = await _send('GET', '/api/v1/courier/jobs', query: {
      if (lat != null) 'lat': '$lat',
      if (lng != null) 'lng': '$lng',
    });
  ...
  Future<Map<String, dynamic>> courierJob(String offerId) => _send('GET', '/api/v1/courier/jobs/$offerId');

  Future<Map<String, dynamic>> respondToOffer(String offerId, {required bool accept, String? reason}) => _send(
        'POST',
        '/api/v1/courier/offers/$offerId/respond',
        body: {'response': accept ? 'ACCEPTED' : 'DECLINED', if (reason != null) 'reason': reason},
      );

  Future<List<DeliveryDto>> courierDeliveries({bool active = false}) async {
    final d = await _send('GET', '/api/v1/courier/deliveries', query: {if (active) 'active': '1'});
    return _items(d, DeliveryDto.fromJson);
  }

  Future<DeliveryDto> courierDelivery(String id) async =>
      DeliveryDto.fromJson(await _send('GET', '/api/v1/courier/deliveries/$id'));

  Future<DeliveryDto> advanceDelivery(String id, String to, {double? lat, double? lng}) async => DeliveryDto.fromJson(
        await _send('POST', '/api/v1/courier/deliveries/$id/advance', body: {
          'to': to,
          if (lat != null) 'lat': lat,
          if (lng != null) 'lng': lng,
        }),
  ...
  Future<DeliveryDto> verifyPickup(String id, {String? code, int? packageCount, String? conditionNote}) async =>
      DeliveryDto.fromJson(await _send('POST', '/api/v1/courier/deliveries/$id/verify-pickup', body: {
        if (code != null) 'code': code,
        if (packageCount != null) 'packageCount': packageCount,
        if (conditionNote != null) 'conditionNote': conditionNote,
      }));

  Future<DeliveryDto> verifyDropoff(String id, {String? code, String? recipientName, String? signatureKey}) async =>
      DeliveryDto.fromJson(await _send('POST', '/api/v1/courier/deliveries/$id/verify-dropoff', body: {
        if (code != null) 'code': code,
        if (recipientName != null) 'recipientName': recipientName,
        if (signatureKey != null) 'signatureKey': signatureKey,
      }));

  Future<DeliveryDto> submitPod(String id, {required List<String> photoKeys, String? notes}) async =>
      DeliveryDto.fromJson(await _send('POST', '/api/v1/courier/deliveries/$id/pod', body: {
        'photoKeys': photoKeys,
        if (notes != null) 'notes': notes,
      }));

  Future<DeliveryDto> failDelivery(String id, {required String reason}) async =>
      DeliveryDto.fromJson(await _send('POST', '/api/v1/courier/deliveries/$id/fail', body: {'reason': reason}));

  Future<void> courierBreadcrumb(String id, double lat, double lng, {double? heading, double? speed}) => _send(
        'POST',
        '/api/v1/courier/deliveries/$id/breadcrumb',
        body: {'lat': lat, 'lng': lng, if (heading != null) 'heading': heading, if (speed != null) 'speed': speed},
      );

  Future<void> courierRateCustomer(String id, {required int stars, String? comment}) => _send(
        'POST',
  ...
  Future<EarningsSummaryDto> courierEarnings() async =>
      EarningsSummaryDto.fromJson(await _send('GET', '/api/v1/courier/earnings'));

  Future<List<EarningTxnDto>> courierEarningTxns({String? cursor}) async {
    final d = await _send('GET', '/api/v1/courier/earnings/transactions',
        query: {if (cursor != null) 'cursor': cursor});
    return _items(d, EarningTxnDto.fromJson);
  }

  Future<int> courierRequestPayout({required int amountMinor, required String pin}) async {
    final d = await _send('POST', '/api/v1/courier/payouts', body: {'amountMinor': amountMinor, 'pin': pin});
    return (d['balanceMinor'] as num?)?.toInt() ?? 0;
  }
  ...
  Future<List<SellerOrderDto>> sellerOrders({String? status}) async {
    final d = await _send('GET', '/api/v1/vendors/orders', query: {if (status != null) 'status': status});
    return _items(d, SellerOrderDto.fromJson);
  }

  Future<SellerOrderDetailDto> sellerOrder(String id) async =>
      SellerOrderDetailDto.fromJson(await _send('GET', '/api/v1/vendors/orders/$id'));

  /// Advance a sub-order (`ACCEPTED` → … → `HANDED_OVER`). Returns the new status
  /// and, once a delivery is spawned, its id.
  Future<({String status, String? deliveryId})> advanceSellerOrder(String id, String status) async {
    final d = await _send('PATCH', '/api/v1/vendors/orders/$id', body: {'status': status});
    return (status: (d['status'] as String?) ?? status, deliveryId: d['deliveryId'] as String?);
  }

  /// Mark the sub-order COMPLETED — releases the seller payout from escrow.
  Future<void> completeSellerOrder(String id) => _send('POST', '/api/v1/vendors/orders/$id/complete');

  /// The seller's return queue.
  Future<List<VendorReturnDto>> sellerReturns({String? status}) async {
    final d = await _send('GET', '/api/v1/vendors/returns', query: {if (status != null) 'status': status});
    return _items(d, VendorReturnDto.fromJson);
  }

  /// Approve (refunds the customer) or reject a return.
  Future<({String status, int? refundAmountMinor, String? refundMethod})> reviewReturn(
    String returnId, {
    required String decision,
    String? note,
  }) async {
    final d = await _send('POST', '/api/v1/vendors/returns/$returnId/review', body: {
      'decision': decision,
  ...
  Future<int> vendorRequestPayout({required int amountMinor, required String pin}) async {
    final d = await _send('POST', '/api/v1/vendors/payouts', body: {'amountMinor': amountMinor, 'pin': pin});
    return (d['balanceMinor'] as num?)?.toInt() ?? 0;
  }
  ...
  Future<DeliveryDto> vendorDelivery(String id) async =>
      DeliveryDto.fromJson(await _send('GET', '/api/v1/vendors/deliveries/$id'));

  // --- Phase 5: Inverse Draws / auctions ----------------------

  Future<List<AuctionCardDto>> auctions({String? status}) async {
    final d = await _send('GET', '/api/v1/auctions', auth: false, query: {if (status != null) 'status': status});
    return _items(d, AuctionCardDto.fromJson);
  }

  Future<AuctionDetailDto> auction(String slug) async =>
      AuctionDetailDto.fromJson(await _send('GET', '/api/v1/auctions/$slug', auth: true));

  Future<List<({int rank, String name, int ticketCount, double score})>> auctionLeaderboard(String slug, {int limit = 20}) async {
    final d = await _send('GET', '/api/v1/auctions/$slug/leaderboard', auth: false, query: {'limit': '$limit'});
    return (d['items'] as List<dynamic>? ?? const [])
  ...
  Future<QualificationDto> auctionQualification(String slug) async =>
      QualificationDto.fromJson(await _send('GET', '/api/v1/auctions/$slug/qualification'));

  Future<double> auctionQualify(String slug, {required String factor, String? key, double? points}) async {
    final d = await _send('POST', '/api/v1/auctions/$slug/qualify', body: {
      'factor': factor,
      if (key != null) 'key': key,
  ...
  Future<Map<String, dynamic>> buyTickets(String slug, {String? packageId, int? count, required String paymentMethod, String? gateway}) =>
      _send('POST', '/api/v1/auctions/$slug/tickets',
          extraHeaders: {'Idempotency-Key': 'seats-${DateTime.now().microsecondsSinceEpoch}'},
          body: {
  ...
  Future<List<({String serial, int? seatNo, String source, String status})>> myTicketsFor(String slug) async {
  ...
  Future<String> conversationForOrder(String orderId) async {
    final d = await _send('POST', '/api/v1/conversations/for-order', body: {'orderId': orderId});
    return d['conversationId'] as String;
  }

  Future<String> conversationForDelivery(String deliveryId) async {
    final d = await _send('POST', '/api/v1/conversations/for-delivery', body: {'deliveryId': deliveryId});
    return d['conversationId'] as String;
  }

  Future<({List<ChatMessageDto> items, String? nextCursor})> messages(String conversationId, {String? cursor}) async {
    final d = await _send('GET', '/api/v1/conversations/$conversationId/messages', query: {if (cursor != null) 'cursor': cursor});
    return (items: _items(d, ChatMessageDto.fromJson), nextCursor: d['nextCursor'] as String?);
  }

  Future<void> sendMessage(String conversationId, {String? body, Object? attachments, String? kind}) =>
      _send('POST', '/api/v1/conversations/$conversationId/messages', body: {
  ...
  Future<void> blockUser(String targetUserId) => _send('POST', '/api/v1/me/blocks', body: {'targetUserId': targetUserId});

  // --- Phase 6: notifications ------------------------------------

  Future<NotificationFeedDto> notifications({bool unreadOnly = false, String? cursor}) async => NotificationFeedDto.fromJson(
        await _send('GET', '/api/v1/notifications', query: {
          if (unreadOnly) 'unread': '1',
          if (cursor != null) 'cursor': cursor,
        }),
      );

  Future<void> markNotificationRead(String id) => _send('POST', '/api/v1/notifications/$id/read');
  Future<void> markAllNotificationsRead() => _send('POST', '/api/v1/notifications/read-all');
  ...
  Future<List<NotificationPrefDto>> setNotificationPreference(String category, {bool? push, bool? email, bool? sms, bool? inApp}) async {
    final d = await _send('PATCH', '/api/v1/notifications/preferences', body: {
      'category': category,
  ...
  Future<DisputeDto> dispute(String id) async => DisputeDto.fromJson(await _send('GET', '/api/v1/disputes/$id'));

  Future<Map<String, dynamic>> openDispute({required String kind, required String refId, required String category, required String body}) =>
      _send('POST', '/api/v1/disputes', body: {'kind': kind, 'refId': refId, 'category': category, 'body': body});

  Future<void> addDisputeEvidence(String id, {String? fileKey, String? body, String? kind}) =>
      _send('POST', '/api/v1/disputes/$id/evidence', body: {
        if (kind != null) 'kind': kind,
  ...
  Future<void> submitReport({required String targetType, required String targetId, required String category, required String body}) =>
      _send('POST', '/api/v1/reports', body: {'targetType': targetType, 'targetId': targetId, 'category': category, 'body': body});
  ...
  Future<HelpCenterDto> helpCenter() async =>
      HelpCenterDto.fromJson(await _send('GET', '/api/v1/support/help', auth: false));
  ...
  Future<SupportTicketDto> supportTicket(String id) async =>
      SupportTicketDto.fromJson(await _send('GET', '/api/v1/support/tickets/$id'));

  Future<Map<String, dynamic>> createSupportTicket({required String category, required String subject, required String body, String? priority}) =>
      _send('POST', '/api/v1/support/tickets', body: {
        'category': category,
        'subject': subject,
        'body': body,
  ...
  Future<void> auctionSubmitClaimKyc(String claimId, {required List<Map<String, String>> documents}) =>
      _send('POST', '/api/v1/auctions/claims/$claimId/kyc', body: {'documents': documents});

  Future<Map<String, dynamic>> auctionWinPurchase(String slug, {required String paymentMethod, String? gateway}) =>
      _send('POST', '/api/v1/auctions/$slug/win-purchase',
          extraHeaders: {'Idempotency-Key': 'winbuy-${DateTime.now().microsecondsSinceEpoch}'},
          body: {'payment': {'method': paymentMethod, if (gateway != null) 'gateway': gateway}});
  ...
  Future<List<SponsoredCardDto>> sponsored({required String slot, String? categoryId, int? limit}) async {
    final d = await _send('GET', '/api/v1/ads/sponsored', auth: false, query: {
      'slot': slot,
  ...
  Future<void> logAdEvent({required String campaignId, String? adId, required String kind, String? placement}) =>
      _send('POST', '/api/v1/ads/events', auth: false, body: {
  ...
  Future<List<CampaignDto>> campaigns({String? status}) async {
    final d = await _send('GET', '/api/v1/vendors/campaigns', query: {if (status != null) 'status': status});
    return _items(d, CampaignDto.fromJson);
  }

  Future<CampaignDetailDto> campaign(String id) async =>
      CampaignDetailDto.fromJson(await _send('GET', '/api/v1/vendors/campaigns/$id'));
  ...
  Future<CampaignDto> createCampaign({
    ...
    Map<String, dynamic>? targeting,
      ...
      CampaignDto.fromJson(await _send('POST', '/api/v1/vendors/campaigns', body: {
        'name': name,
  ...
  Future<void> setCampaignProducts(String id, List<String> productIds) =>
      _send('POST', '/api/v1/vendors/campaigns/$id/products', body: {'productIds': productIds});
  ...
  Future<void> addCampaignCreative(
    ...
    int? weight,
      ...
      _send('POST', '/api/v1/vendors/campaigns/$id/creatives', body: {
        'slot': slot,
  ...
  Future<void> updateCampaignCreative(
    ...
    bool? isActive,
      ...
      _send('PATCH', '/api/v1/vendors/campaigns/$id/creatives/$adId', body: {
  ...
  Future<CampaignDto> submitCampaign(String id, {required String paymentMethod, String? gateway}) async =>
      CampaignDto.fromJson(await _send('POST', '/api/v1/vendors/campaigns/$id/submit',
          extraHeaders: {'Idempotency-Key': 'camp-sub-$id'},
          body: {'payment': {'method': paymentMethod, if (gateway != null) 'gateway': gateway}}));

  Future<CampaignDto> pauseCampaign(String id) async => CampaignDto.fromJson(await _send('POST', '/api/v1/vendors/campaigns/$id/pause'));
  Future<CampaignDto> resumeCampaign(String id) async => CampaignDto.fromJson(await _send('POST', '/api/v1/vendors/campaigns/$id/resume'));
  Future<CampaignDto> cancelCampaign(String id) async => CampaignDto.fromJson(await _send('POST', '/api/v1/vendors/campaigns/$id/cancel'));

  // --- Phase 7: analytics ------------------------------------------

  Future<VendorAnalyticsDto> vendorAnalytics({int days = 30}) async =>
      VendorAnalyticsDto.fromJson(await _send('GET', '/api/v1/vendors/analytics', query: {'days': '$days'}));

  Future<CourierAnalyticsDto> courierAnalytics({int days = 30}) async =>
      CourierAnalyticsDto.fromJson(await _send('GET', '/api/v1/courier/analytics', query: {'days': '$days'}));
  ...
  Future<void> applyReferralCode(String code, {String? channel}) =>
      _send('POST', '/api/v1/me/referrals/apply', body: {'code': code, if (channel != null) 'channel': channel});
}
