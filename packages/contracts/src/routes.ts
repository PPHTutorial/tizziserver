...
export const routes = {
  ...
  catalogCategories: {
    ...
    summary: "Tenant category list (flat, or `?tree=1` nested, or `?filter=` ranked roots)",
    tags: ["catalog"],
    auth: false,
    query: z.object({
  ...
  catalogScan: {
    method: "GET",
    path: "/api/v1/catalog/scan",
    summary: "Barcode/SKU scan → product slug",
    tags: ["catalog"],
    auth: false,
    query: z.object({ code: z.string().min(1) }),
    response: c.ScanResponse,
  },
  catalogProducts: {
    method: "GET",
  ...
  catalogReviewsList: {
    ...
    summary: "Paginated review list + a 1-5★ distribution",
    tags: ["catalog"],
    auth: false,
    query: z.object({ cursor: z.string().optional(), limit: z.coerce.number().int().positive().max(50).optional() }),
    response: c.ReviewsListResponse,
    errors: [404],
  },
  catalogReview: {
    method: "POST",
  ...
  vendorGetProduct: {
    ...
    summary: "Full editable detail for one of the vendor's products",
    tags: ["vendors"],
    auth: true,
    response: c.MyProductDetailResponse,
    errors: [403, 404],
  },
  vendorUpdateProduct: {
    method: "PATCH",
  ...
  vendorPauseProduct: {
    method: "POST",
    path: "/api/v1/vendors/products/{id}/pause",
    summary: "Pause/resume the vendor's own offer for this listing",
    tags: ["vendors"],
    auth: true,
    body: c.SetListingPausedRequest,
    response: c.ProductMutationResponse,
    errors: [403, 404, 409],
  },
  vendorArchiveProduct: {
    method: "DELETE",
    path: "/api/v1/vendors/products/{id}",
    summary: "Archive (soft-delete) a listing",
    tags: ["vendors"],
    auth: true,
    response: c.ProductMutationResponse,
    errors: [403, 404],
  ...
  recentlyViewedClear: {
    method: "DELETE",
    path: "/api/v1/me/recently-viewed",
    summary: "Clear the caller's recently-viewed history",
    tags: ["me"],
    auth: true,
    response: ok(z.object({ cleared: z.boolean() })),
  },
  ...
  myTicketStats: { method: "GET", path: "/api/v1/me/tickets/stats", summary: "Profile-card summary: total tickets, active entries, amount won", tags: ["auctions"], auth: true, response: x.MyTicketStatsResponse, errors: [403] },
  ...
  conversationForVendor: { method: "POST", path: "/api/v1/conversations/for-vendor", summary: "Open/reuse the customer↔vendor thread from a vendor's storefront", tags: ["chat"], auth: true, body: z.object({ vendorId: z.string() }), response: k.ConversationRefResponse, errors: [404] },
